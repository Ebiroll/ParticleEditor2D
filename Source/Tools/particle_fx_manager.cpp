...
namespace vis
{
       ...
       uint32_t seed = 4711;
        ...
        std::shared_ptr<Particle_fx_resource> load_pfx_resource(const char* file)
        {
            ...
            //data.back() = 0;
            data[size]=0;
            //std::shared_ptr<Particle_fx_resource> ptr=context.create_pfx_resource(file, &data[0], data.size()-1);
            std::shared_ptr<Particle_fx_resource> ptr=context.create_pfx_resource(file, &data[0], size);
            assert(data[size]==0);
            //free(data);
            //assert(data.back() == 0);
            return ptr;
